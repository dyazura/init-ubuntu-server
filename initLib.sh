...
# New User Add
#   Account Username
#   Flag to determine if user account is added silently
...
# Add local pubKey for the new user account
#   Account Username
#   Public SSH Key
function addSSHKey() {
    local username=${1}
...
# Create the swap file, Maximum size of swap is 2GB
function createSwap() {
   local swapmem=$(($(getPhysicalMemory) * 2))

   if [ ${swapmem} -gt 2 ]; then
        swapmem=2
   fi
